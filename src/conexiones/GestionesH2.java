...
public class GestionesH2 {
	...
	private void listadoPacientesCompleto() throws SQLException {
		...
		while (miResultSet.next()) {
			System.out.println("id: " + miResultSet.getInt(1) + ", dni: " + miResultSet.getString(2) + ", nombre: "
					...
					+ miResultSet.getString(5) + ", edad: " + miResultSet.getString(6) + ", profesion: "
	...
	private void listadoPacientesSinCitaCompletada() throws SQLException {
		...
		ResultSet miResultSet = miSentencia.executeQuery(
				"SELECT p.dni, p.nombre FROM recuperacionad.paciente p , recuperacionad.consulta c where p.baja = 0 and  c.realizada=0");
		while (miResultSet.next()) {
	...
	private void listadoHistoricoPacientes() throws SQLException {
		...
		ResultSet miResultSet = miSentencia.executeQuery(
				"SELECT c.* FROM recuperacionad.paciente p , recuperacionad.consulta c where p.baja = 0 and c.realizada=1 and p.idPaciente = "
						+ id);
		while (miResultSet.next()) {
